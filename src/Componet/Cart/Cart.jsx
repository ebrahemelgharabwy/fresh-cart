...
export default function Cart() {
  ...
  let { getCartItems, removeCartItem, updatCartItem } = useContext(CartContecxt);
  ...
  async function updatItem(productId, count) {
    if (count < 1)
      removeItem(productId)
    let response = await updatCartItem(productId, count)
    setCartDetails(response.data);
...
{loading ? (
    ...
    <div className="relative  shadow-green-500 my-4 pt-10 overflow-x-auto shadow-md sm:rounded-lg">
      
      <div className='flex justify-end text-center py-4'> <h2 className='text-3xl font-semibold ps-4'>cart shop</h2>
        <Link  to={'/AllOrders'} className=' text-white me-4 py-2 mx-auto px-6 bg-green-500 rounded-lg'>Check Out</Link>
      </div>
